...
SELECT COUNT(*) INTO reservations_number FROM reservation INNER JOIN trip ON res_tr_id=tr_id WHERE code=tr_br_code;

SELECT trip.tr_cost,trip.tr_maxseats,(SELECT reservations_number) reservations,
(SELECT trip.tr_maxseats-reservations_number 
FROM trip WHERE code=tr_br_code) empty_seats,
drv.lname AS driver_lname,drv.name AS driver_name,
gui.lname AS guide_lname,gui.name AS guide_name,
trip.tr_departure,trip.tr_return FROM trip INNER JOIN worker AS drv 
ON tr_br_code=drv.wrk_br_code 
INNER JOIN worker AS gui 
ON tr_br_code=gui.wrk_br_code 
